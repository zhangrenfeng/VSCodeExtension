...
connection.onCompletion((textDocumentPosition, token) => {
    ...
    return runner_1.runSafeAsync(() => __awaiter(this, void 0, void 0, function* () {
        ...
        if (document) {
            ...
            if (appPath && appMap[appPath]) {
                const appInfo = appMap[appPath];
                const item = appInfo.items[filePath.slice(0, -5)];
                if (item && loc.path.length === 2 && loc.path[0] === 'usingComponent' && typeof loc.path[1] === 'string') {
                    const components = Object.keys(appInfo.items).filter(k => appInfo.items[k].isComponent);
                    // return pages.map(p => {
                    //   const page = path.relative(appPath, p)
                    ...
                    pagesNode.children.forEach(child => {
                        ...
                        if (pages.indexOf(pagePath) >= 0) {
                            ...
                            links.push({
                                ...
                                target: `command:kobex.createPage?${encodeURIComponent(JSON.stringify([pagePath]))}`
                            });
                        }
                    });
                    return links;

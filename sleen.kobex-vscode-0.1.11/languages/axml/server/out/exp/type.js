...
Type._dummy = Type.Number.registerPropertys({
    ...
    "toFixed": new Method(Type.Number, '返回数字的定点数表示形式', [new Parameter('fractionDigits', Type.Number, '保留小数位数')], (n, fractionDigits) => n.toFixed(fractionDigits)),
    "toPrecision": new Method(Type.Number, '返回数字的浮点数表示，指定有效数字', [new Parameter('precision', Type.Number, '有效数字位数')], (n, precision) => n.toPrecision(precision)),
});

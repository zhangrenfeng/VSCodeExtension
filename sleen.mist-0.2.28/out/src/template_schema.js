...
schema_1.SchemaFormat.registerFormat('color', {
    validateJsonNode(node, offset, matchingSchemas) {
        return schema_1.validateJsonNode(node, colorSchema, offset, matchingSchemas);
    }
});
schema_1.SchemaFormat.registerFormat('event', {
    validateJsonNode(node, offset, matchingSchemas) {
        return schema_1.validateJsonNode(node, NodeSchema.getEventSchema(), offset, matchingSchemas);
    }
});
class NodeSchema {
    static setCurrentDir(dir) {
        return __awaiter(this, void 0, void 0, function* () {
            if (this.currentDir !== dir) {
                ...
                if (!config) {
                    let config = yield this.readConfigInDir(dir);
    ...
    static readConfig(data, dir) {
        if (data.extends) {
            let uri = data.extends;
            delete data.extends;
            ...
            return this.readUri(uri).catch(error => '{}').then(content => {
                ...
                return Promise.all([this.readConfig(extendsData, path.dirname(uri)), this.readConfig(data, dir)]).then(r => {
                    ...
                    config.types = Object.assign({}, extendsConfig.types, config.types);
                    config.actions = Object.assign({}, extendsConfig.actions, config.actions);
                    config.properties = Object.assign({}, extendsConfig.properties, config.properties);
                    config.styleProperties = Object.assign({}, extendsConfig.styleProperties, config.styleProperties);
                    return config;
                });
            });
        }
        let definitions = data.definitions;
        definitions = schema_1.parseSchema(definitions, definitions);
        let readPropertiesMap = map => {
            if (!map)
                return {};
            Object.keys(map).forEach(p => {
                map[p] = schema_1.parseSchema(map[p], definitions);
            });
            return map;
        };
    ...
    static getEventSchema() {
        let config = this.getConfig();
        return {
            type: "object",
            additionalProperties: true,
            properties: Object.assign({ "openUrl:": SimpleSchema("string", "打开指定的 URL"), "updateState:": SimpleSchema("object", "更新状态。值应该为一个字典，将状态中对应的值更新。注意不是替换整个状态，只是更改对应的 key"), "alert:": {
                    oneOf: [
    ...
    static getTypes() {
        ...
        return Object.assign({}, this.nodeTypes, config.types);
    }
    getSchema(node) {
        if (node && node.type === 'object') {
            ...
            if (!schema) {
                let s = {
                    ...
                    properties: Object.assign({}, propertiesMap.common || {}, propertiesMap[type] || {}, { style: {
                            type: 'object',
                            additionalProperties: isCustomType,
                            properties: Object.assign({}, stylesMap.common || {}, stylesMap[type] || {}, config.styleProperties['common'] || {}, config.styleProperties[type] || {}),
                            description: "元素的样式和布局属性\n\n[查看文档](https://vizzle.github.io/MIST/basics/Property.html#style)"
                        } }, config.properties['common'] || {}, config.properties[type] || {}, { "type": {
                            oneOf: [
                ...
                events.forEach(e => {
                    ...
                    s.properties[e + '-once'] = Object.assign({}, schema);
...
${schema.description}`;
    ...
    validateJsonNode(node, offset, matchingSchemas) {
        return schema_1.validateJsonNode(node, this.getSchema(node), offset, matchingSchemas);
    }
}
NodeSchema.nodeSchemaCache = {};
NodeSchema.configs = {};
NodeSchema.nodeTypes = {
...
schema_1.SchemaFormat.registerFormat('node', new NodeSchema());
const VariablesTableSchema = {
    type: "object",
    patternProperties: {
        "^[a-zA-Z_][a-zA-Z0-9_]*$": {
            additionalProperties: true
...
function SimpleSchema(type, description) {
    return { type, description, additionalProperties: true };
}
function EnumSchema(enums, description) {
    let schema = {
        type: "string",
        description
    };
...
function LengthSchema(percentage, enums, description) {
    return {
        oneOf: [
            { type: "number" },
...
function EventSchema(description) {
    return {
        type: 'object',
        format: 'event',
        description,
...
const textViewCommon = Object.assign({}, textCommon, { "auto-focus": SimpleSchema("boolean", "自动获取焦点，默认为 `false`"), "editable": SimpleSchema("boolean", "是否可编辑，默认为 `false`"), "max-length": {
        type: "number",
        min: 0,
        description: "最大输入长度，默认为 `-1`，不限制输入长度"
    }, "placeholder": SimpleSchema("string", "没有输入文本时的提示文字"), "placeholder-color": ColorSchema("提示文字的颜色"), "keyboard-type": EnumSchema({
    ...
    }, "键盘类型，默认为 `default`"), "keyboard-appearance": EnumSchema(["default", "dark", "light"], "键盘外观，默认为 `default`"), "return-key-type": EnumSchema([
        "default",
        "go",
        "google",
        "join",
        ...
        "done",
        "emergency-call"
    ], "返回按键文本，默认为 `default`"), "blur-on-submit": SimpleSchema("boolean", "点击返回按键是否失去焦点，默认为 `true`") });
const stylesMap = {
    common: {
...
[查看文档](https://vizzle.github.io/MIST/basics/Style.html#corner-radius)`),
        ...
        "properties": {
            type: "object",
            additionalProperties: true,
            properties: Object.keys(viewProperties).reduce((p, c) => (Object.assign({}, p, { [c]: {
...
[查看文档](https://vizzle.github.io/MIST/components/paging.html#page-control-margin-left-page-control-margin-right-page-control-margin-top-page-control-margin-bottom)`),
    ...
    "text-field": Object.assign({}, textViewCommon, { "password-mode": SimpleSchema("boolean", "密码输入模式"), "clear-button-mode": EnumSchema([
            "never",
            "while-editing",
            "unless-editing",
            "always"
        ], "清除按钮模式，默认为 `never`") }),
    "text-view": Object.assign({}, textViewCommon),
    switch: {
...
exports.templateSchema = schema_1.parseSchema({
    "definitions": {
